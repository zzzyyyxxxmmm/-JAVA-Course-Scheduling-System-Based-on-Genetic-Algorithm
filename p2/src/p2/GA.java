...
public class GA {
	...
	public void mutation() {
		...
		while(w!=0){
		int position1 = random.nextInt(29);
		int position2 = random.nextInt(mis);
		double k = random.nextDouble();
		//如果产生的随机数ｋ小于变异率，则进行变异操作。
		if (0.5 > k) {
			mutation(position1, position2);
		}
...
static 	public  Object  [][]cellData=new Object[100][10];
	...
	boolean check(int x,int y,int cl,int z){	//检查是否和之前排过的课程发生冲突
		...
		for(int ii=0;ii<3;ii++){
			...
			for(j=1;j<=4;j++){
				for(k=0;k<5;k++){
					if(aa[i][j][k].cl==null)	continue;
					//System.out.println(aa[i][j][k].cl+" "+aa[i][j][k].cname+" "+i+" "+j+" "+k+ww.cname+ww.cl);
					if(aa[i][j][k].cl.equals(ww.cl)&&aa[i][j][k].cname.equals(ww.cname)){			//在间隔不到一天的时间内，存在给相同的班上相同的课
						flag=false;
					}
				}
			}
	...
	public void read(){
			    ...
			    try
			    {
String Dir="C:\\Users\\Administrator\\Desktop\\java project\\1.xls";
		       ...
		       public void sp(String s1,String s2,String s3,String ss1,String ss2)	//教师 班级 时间            时间 教室
		       {
    				   ...
    				   int iii=s3.charAt(2)-'0'-1;
			    			   ...
			    			   {	//找到班级前缀
			    				   ...
			    				   String ww=s1+"老师在"+ss2+"教室教"+s2+mmap.get((String)rmapt.get(s1));
